...
with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    ...
    print("Welcome to Chat GTC, the Grant and Trevor Client.\n"
          "Please use the command '%connect <host_addr:port_#> <username>' to connect to your desired server.\n")
    while True:
        while True:
        ...
        while True:
            # Allow the user to input any command/message whenever they want
            message = input("Type a command to send to the server: ")
            ...
            # Output the server response
            print('Server sent:', data.decode('utf-8'))
            if data == b"Disconnected from server.":  # If the user exits, they return to the start
                break
